...
for trial=1:Trials
    h=[];
    for i=1:120  % Hay 20 hits en cada trial, 20*35 = 700 que es el tamaño de a
        vl=stims(b((trial-1)*120+i)+1,1:2);
        %[(trial-1)*Trials+i vl(2)]
        if (H(trial,1) == vl(2)) || (H(trial,2) == vl(2))
            stims(b((trial-1)*120+i),2) = 33285;
        end
    end
    h = unique(h);
    [trial h]
...
assert( size(validstimuls,2) == size(stimulations,1), 'There are some stimulus that has been deleted. Check!');
